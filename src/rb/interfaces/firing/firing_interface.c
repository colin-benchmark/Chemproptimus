...
status_t firing_write_handler(
    ...
    if (instance_n >= FIRING_INSTANCES_N) {
...
status_t firing_method_handler(uint8_t instance_n, uint8_t method_id, uint8_t *input, uint8_t input_len) {
    ...
    switch (method_id) {
        case FIRING_METHOD_FIRE_SHOTS_ID:
            ...
            status = firing_fire_shots(firing_fire_shots_parameters.thruster, firing_fire_shots_parameters.shots);
            break;
        ...
        case FIRING_METHOD_FIRE_CYCLES_ID:
            ...
            status = firing_fire_cycles(firing_fire_cycles_parameters.cycles);
            break;
