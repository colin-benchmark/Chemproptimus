...
static int csp_rdp_send_cmp(csp_conn_t *conn, csp_packet_t *packet, int flags, int seq_nr, int ack_nr) {
    ...
    csp_rdp_protocol(
        "RDP %p: Send CMP S %u: syn %u, ack %u, eack %u, rst %u, seq_nr %5u, ack_nr %5u, packet_len %u (%u)\n",
        conn,
        conn->rdp.state,
        ((header->flags & RDP_SYN) != 0),
        ((header->flags & RDP_ACK) != 0),
        ((header->flags & RDP_EAK) != 0),
        ((header->flags & RDP_RST) != 0),
        be16toh(header->seq_nr),
        be16toh(header->ack_nr),
        packet->length,
        (unsigned int)(packet->length - sizeof(rdp_header_t))
    );
...
static void csp_rdp_flush_eack(csp_conn_t *conn, csp_packet_t *eack_packet) {
    ...
    for (i = 0; i < count; i++) {
        ...
        rdp_header_t *header = csp_rdp_header_ref((csp_packet_t *)packet);
        csp_rdp_protocol(
            "RDP %p: EACK compare element, time %" PRIu32 ", seq %u\n",
            conn,
            packet->timestamp_tx,
            be16toh(header->seq_nr)
        );
...
void csp_rdp_check_timeouts(csp_conn_t *conn) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (csp_rdp_seq_before(be16toh(header->seq_nr), conn->rdp.snd_una)) {
            csp_rdp_protocol(
                "RDP %p: TX Element Free, time %" PRIu32 ", seq %u, una %u\n",
                conn,
                packet->timestamp_tx,
                be16toh(header->seq_nr),
                conn->rdp.snd_una
...
bool csp_rdp_new_packet(csp_conn_t *conn, csp_packet_t *packet) {
    ...
    csp_rdp_protocol(
        ...
        conn,
        conn->rdp.state,
        ((rx_header->flags & RDP_SYN) != 0),
        ((rx_header->flags & RDP_ACK) != 0),
        ((rx_header->flags & RDP_EAK) != 0),
        ((rx_header->flags & RDP_RST) != 0),
        rx_header->seq_nr,
        rx_header->ack_nr,
        packet->length,
        (unsigned int)(packet->length - sizeof(rdp_header_t))
    ...
    if (rx_header->flags & RDP_RST) {
        ...
        if (conn->rdp.state == RDP_CLOSE_WAIT) {
            csp_rdp_protocol(
                "RDP %p: RST received in CLOSE_WAIT, ack: %d - closing\n", conn, (rx_header->flags & RDP_ACK)
            );
    ...
    switch (conn->rdp.state) {
        ...
        case RDP_CLOSED: {
            ...
            csp_rdp_protocol(
                "RDP %p: window size %" PRIu32 ", conn timeout %" PRIu32 ", packet timeout %" PRIu32
                ", delayed acks: %" PRIu32 ", ack timeout %" PRIu32 ", ack each %" PRIu32 " packet\n",
                conn,
        ...
        case RDP_OPEN: {
            ...
            if (!csp_rdp_seq_between(
                    rx_header->seq_nr, conn->rdp.rcv_cur + 1, conn->rdp.rcv_cur + (conn->rdp.window_size * 2)
                )) {
                csp_rdp_protocol(
                    "RDP %p: Invalid sequence number! %u not between %u and %" PRIu32 "\n",
                    conn,
                    rx_header->seq_nr,
                    conn->rdp.rcv_cur + 1U,
                    conn->rdp.rcv_cur + (conn->rdp.window_size * 2U)
                );
            ...
            if (!csp_rdp_seq_between(
                ...
                csp_rdp_error(
                    "RDP %p: Invalid ACK number! %u not between %" PRIu32 " and %u\n",
                    conn,
                    rx_header->ack_nr,
                    conn->rdp.snd_una - 1 - (conn->rdp.window_size * 2),
                    conn->rdp.snd_nxt - 1
                );
                goto discard_open;
            }
...
int csp_rdp_send(csp_conn_t *conn, csp_packet_t *packet) {
    ...
    csp_rdp_protocol(
        ...
        conn,
        conn->rdp.state,
        (tx_header->flags & RDP_SYN),
        (tx_header->flags & RDP_ACK),
        (tx_header->flags & RDP_EAK),
        (tx_header->flags & RDP_RST),
        be16toh(tx_header->seq_nr),
        be16toh(tx_header->ack_nr),
        packet->length,
        (unsigned int)(packet->length - sizeof(rdp_header_t))
    );
...
static int csp_rdp_close_internal(csp_conn_t *conn, uint8_t closed_by, bool send_rst) {
    ...
    if (conn->rdp.state != RDP_CLOSE_WAIT) {
        ...
        csp_rdp_protocol(
            "RDP %p: csp_rdp_close(0x%x)%s -> CLOSE_WAIT\n", conn, closed_by, send_rst ? ", sent RST" : ""
        );
    ...
    if (conn->rdp.closed_by != CSP_RDP_CLOSED_BY_ALL) {
        csp_rdp_protocol(
            "RDP %p: csp_rdp_close(0x%x) != %x, waiting for:%s%s%s\n",
            conn,
            closed_by,
            conn->rdp.closed_by,
